	...
	internal static class VitalsPatches
	{
		...
		public static class VitalsGuiPatchBegin
		{
			public static void Postfix(VitalsMinigame __instance)
			{
				...
				VitalsPanel[] vitalsPanels = __instance.Field_3;
		...
		public static class VitalsGuiPatchUpdate
		{
			public static void Postfix(VitalsMinigame __instance)
            {
                if (PlayerTask.PlayerHasTaskOfType<HudOverrideSystemType>(PlayerControl.LocalPlayer))
                    return;
				...
				VitalsPanel[] vitalsPanels = __instance.Field_3.OrderBy(x => (x.IsDead ? 0 : 1) + (x.IsDiscon ? 2 : 0)).ToArray();//VitalsPanel[] //Sorted by: Dead -> Alive -> dead&disc -> alive&disc
				int i = 0;
